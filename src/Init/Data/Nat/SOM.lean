  ...
  go (fuel : Nat) (p₁ p₂ : Poly) : (Poly.add.go fuel p₁ p₂).denote ctx = p₁.denote ctx + p₂.denote ctx := by
    ...
    | succ _ ih =>
      ...
      next k₁ m₁ p₁ k₂ m₂ p₂ =>
        ...
        · simp (config := { decide := true }) [← Nat.add_assoc, ← Nat.right_distrib, eq_of_beq heq]
        · simp (config := { decide := true }) [Nat.right_distrib, Nat.add_assoc, Nat.add_comm, Nat.add_left_comm]
